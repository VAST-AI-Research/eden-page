...
> Project 和 Blog 目前是隐藏的（`nav` 条目上标了 `hidden: true`）：不进 header、
> 也不在首页出现，但 `?s=` 和 `?p=` 仍然能直达。删掉那一行就放出来了。
...
`authors[]` 还认 `bold: true`，把那个名字加粗（共一 / 项目负责人 / 通讯用）。
只加粗名字，不含 `*` `†` `✉` 这些标注符号。
...
要用自定义域名：仓库根目录放一个 `CNAME` 文件、内容就是域名那一行，然后把 DNS
指向 GitHub Pages（根域名配 4 条 `A` 记录，子域名配一条 `CNAME`）。
本仓库所有路径都是相对的、路由用 `location.pathname` 拼，所以子路径部署和
自定义域名都不需要改任何代码。
